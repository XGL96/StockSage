...
class TestSummarizeFgExperts:
    def test_summarizes_valid_experts(self, summarizer: ResultSummarizer) -> None:
        fg_result: dict[str, Any] = {
            "sentiment": "A" * 30,
            "risk": "B" * 30,
            "technical": "short",  # <= 20 chars, skipped
        }
    ...
    def test_experts_nested_under_research_results(self, summarizer: ResultSummarizer) -> None:
        fg_result: dict[str, Any] = {
            "research_results": {"sentiment": "A" * 30, "hot_money": "B" * 30},
        }
        mock_resp = _make_acompletion_response("nested summary")
    ...
    def test_top_level_takes_precedence_over_nested(self, summarizer: ResultSummarizer) -> None:
        fg_result: dict[str, Any] = {
            "sentiment": "TOP_LEVEL " + "X" * 30,
            "research_results": {"sentiment": "NESTED " + "Y" * 30},
        }
        mock_resp = _make_acompletion_response("summary")

        with patch("stocksage.summarizer.litellm.acompletion", new_callable=AsyncMock, return_value=mock_resp) as mock_call:
            asyncio.run(summarizer.summarize_fg_experts(fg_result, "600519"))
    ...
    def test_mutates_fg_results(self, summarizer: ResultSummarizer) -> None:
        fg_results: dict[str, dict[str, Any]] = {
            "600519": {
                "sentiment": "X" * 30,
                "battle_result": {
                    "debate_history": [{"speaker": "A", "content": "debate content here"}],
                },
            },
        }
